...
-- Drop the existing function if it exists and recreate it with the correct signature
DROP FUNCTION IF EXISTS public.register_for_gd_atomic(uuid, uuid, text, text, text, text, text, text, text, text, text, text);
...
CREATE OR REPLACE FUNCTION public.register_for_gd_atomic(
  ...
  p_participant_occupation TEXT DEFAULT NULL,
  p_participant_occupation_other TEXT DEFAULT NULL,
  p_student_institution TEXT DEFAULT NULL,
  p_student_year TEXT DEFAULT NULL,
  p_professional_company TEXT DEFAULT NULL,
  p_professional_role TEXT DEFAULT NULL,
  p_self_employed_profession TEXT DEFAULT NULL
)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
